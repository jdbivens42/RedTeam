...
# Randomly shuffle the charset to make a custom + polymorphic encoding scheme
Z85CHARS = list(b"0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ.-:+=^!/*?&<>()[]{}@%$#")
random.shuffle(Z85CHARS)
...
decryptor = b''.join(imports).decode() + decryptor.format(encoded=encoded, char_map=repr(Z85MAP), key=repr(key))
print(decryptor)
